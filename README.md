...
| `-c`         | Sets the probability $p_{ct}$ that two randomly selected callsigns will differ only in the least significant byte. Entered as the exponential $x$ in $2^x$, $-32 \geq x \lt 0$. The probability is used to calculate the $\tau$ values and the overall probability of success. |
...
| `-s`         | Set the target probability of success, $0 \lt p_{success} \lt 1$. The default value is $0.5$. |
| `-t`         | Set the $\tau_1$ value. Overrides the value calculated from $p_{ct}$ and $p_{success}$.|
| `-u`         | Set the $\tau_2$ value. The default is set to achieve $p_{success} \geq 0.99$ in the validation steps.|
